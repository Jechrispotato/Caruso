...
function drawSection(targetCtx, state, boxX, boxY, boxW, boxH, scaleFactor, placeholderText) {
    if (state.img) {
        ...
        targetCtx.font = `${24 * scaleFactor}px sans-serif`;
        targetCtx.textAlign = 'center';
        targetCtx.fillText(placeholderText, boxX + (boxW / 2), boxY + (boxH / 2));
    }
}
...
document.getElementById('btn-export').addEventListener('click', async () => {
    ...
    const success = await ipcRenderer.invoke('save-dialog', base64Data);
    if (success) {
        // Flat success state
